...
std::vector<std::shared_ptr<QuadTreeNode>> QuadTreeNode::divide() const {
  ...
  return {std::make_shared<QuadTreeNode>(mLeft, mLeft + mHalfX, mTop,
                                         mTop + mHalfY, featuresTL),
          std::make_shared<QuadTreeNode>(mLeft + mHalfX, mRight, mTop,
                                         mTop + mHalfY, featuresTR),
          std::make_shared<QuadTreeNode>(mLeft, mLeft + mHalfX, mTop + mHalfY,
                                         mBottom, featuresBL),
          std::make_shared<QuadTreeNode>(mLeft + mHalfX, mRight, mTop + mHalfY,
                                         mBottom, featuresBR)};
}

bool QuadTreeNode::isLeaf() const { return mIsLeaf; }

bool QuadTreeNode::isEmpty() const { return mIsEmpty; }
